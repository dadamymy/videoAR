    ...
    if mean_asuna > MIN_MATCHES:
        ...
        if det == "up":
            ...
            if detection == False:
                ...
                if frame_counter == Video_cap.get(cv2.CAP_PROP_FRAME_COUNT) - 4:
                    ...
                    # make video size as big as Target Image
                    video_frame = cv2.resize(video_frame, (w, h))
        frame, video_frame, detection, det = fitting(frame, video_frame, matches_asuna, referenceImagePts, sourceImagePts, corners, det)
        ...
        if det == "up":
            ...
            if detection == False:
                ...
                # print(Video_cap.get(cv2.CAP_PROP_FRAME_COUNT)-3)
                if frame_counter == video_cap_kirito1.get(cv2.CAP_PROP_FRAME_COUNT) - 3:
                    video_cap_kirito1.set(cv2.CAP_PROP_POS_FRAMES, 0)
                    frame_counter = 0
                    det = 0
                    detection = False
            ...
            if detection == False:
                ...
                # print(Video_cap.get(cv2.CAP_PROP_FRAME_COUNT)-3)
                if frame_counter == Video_cap_kirito.get(cv2.CAP_PROP_FRAME_COUNT) - 3:
                    Video_cap_kirito.set(cv2.CAP_PROP_POS_FRAMES, 0)
                    frame_counter = 0
                else:
                    # make video rotate 90 degress
                    ...
                    # make video size as big as Target Image
                    video_frame = cv2.resize(video_frame, (w, h))
        frame, video_frame, detection, det = fitting(frame, video_frame, matches_kirito, referenceImagePts_kirito, sourceImagePts, corners, det)
        cv2.imshow("video", video_frame)
        frame_counter += 1
    # ===================== Display ====================
    # show result
